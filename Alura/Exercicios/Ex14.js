...
function encontrarLivroPorId(id){
    const result = biblioteca.find((livro)=> livro.id === id);
    if(result === undefined) return null;
}

console.log(encontrarLivroPorId(7));
//--------------------------------------------------
...
console.log(filtrarFilmesPorAno(2005));
//--------------------------------------------------
...
function filtrarOrdenarProdutosPorPreco(maxPreco){
    listaFiltrada = listaProdutos.filter((produto)=> produto.preco <= maxPreco);
    listaFiltradaEOrdenada = listaFiltrada.sort((a,b) => {
        if(a["preco"] < b["preco"]) return -1;
        if(a["preco"] > b["preco"]) return 1;
        return 0;
    });
    return listaFiltradaEOrdenada;
...
const animaisOrdenadosPorIdadeDescrescente = ordenarAnimais(compararIdadeDecrescente);
console.log(animaisOrdenadosPorIdadeDescrescente);
//--------------------------------------------------
...
function encontrarFuncionarioPorId(id){
    ...
    if(resultado === undefined) resultado = `Funcionário de id ${id} não cadastrado!`;
    return resultado;
};

const resultado = encontrarFuncionarioPorId(102);
